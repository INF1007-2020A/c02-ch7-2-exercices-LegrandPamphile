...
def build_print_note_name_callback(midi_to_name):
	...
	# Soit message MIDI midi_msg, midi_mesg.type donne le type, midi_msg.note donne la note
...
def build_print_chord_name_callback(chord_names_and_notes, name_to_midi):
	...
	def callback(midi_msg):
		...
		if midi_msg.type == "note_on" and mid_msg.velocity > 0:
			...
			if note_states_tuple in chords:
				...
				print(chord_notes_tuple)
		# Sinon si une note est relâchée
		elif midi_msg.type == "note_off" or (midi_msg.type == "note_on" and mid_msg.velocity == 0):
			# Idem mais à False
